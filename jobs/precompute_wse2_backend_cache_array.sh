#!/bin/bash
#SBATCH -J wse2_ivc_cache
#SBATCH -p serial_requeue
#SBATCH --array=0-440
#SBATCH -t 24:00:00
#SBATCH -c 4
#SBATCH --mem=24G
#SBATCH -o logs/wse2_ivc_cache_%A_%a.out
#SBATCH -e logs/wse2_ivc_cache_%A_%a.err
#SBATCH --requeue
...
SEED_ORDERED_WEIGHT=${SEED_ORDERED_WEIGHT:-"1.0"}
...
if (( TASK_ID >= TOTAL_TASKS )); then
  echo "Task ${TASK_ID} is outside cache mesh size ${TOTAL_TASKS}; exiting."
  exit 0
fi
...
echo "Precomputing WSe2 hysteresis backend cache task ${TASK_ID}/${TOTAL_TASKS} into ${CACHE_ROOT}"
...
python scripts/precompute_wse2_backend_cache.py \
