#!/bin/bash
#SBATCH -J wse2_ivc_hyst_all
#SBATCH -p serial_requeue
#SBATCH --array=0-83
#SBATCH -t 24:00:00
#SBATCH -c 4
#SBATCH --mem=24G
#SBATCH -o logs/wse2_ivc_hyst_all_%A_%a.out
#SBATCH -e logs/wse2_ivc_hyst_all_%A_%a.err
#SBATCH --requeue
...
OUTPUT_ROOT=${OUTPUT_ROOT:-"results/wse2_ivc_hysteresis_linear_interaction_nk24_active2_shell5_theta2_4_u0_20"}
...
echo "Running combined WSe2 IVC hysteresis linecut task ${TASK_ID}/${TOTAL_TASKS} into ${OUTPUT_ROOT}"
echo "Resources: SLURM_CPUS_PER_TASK=${SLURM_CPUS_PER_TASK:-unset} VERTEX_WORKERS=${VERTEX_WORKERS} EXCHANGE_WORKERS=${EXCHANGE_WORKERS} DENSITY_VERTEX_RETENTION=${DENSITY_VERTEX_RETENTION} DENSITY_VERTEX_LAYOUT=${DENSITY_VERTEX_LAYOUT} EXCHANGE_REPRESENTATION=${EXCHANGE_REPRESENTATION} FORM_FACTOR_BACKEND=${FORM_FACTOR_BACKEND} TRIAL_INTERPOLATION=${TRIAL_INTERPOLATION} SLURM_MEM_PER_NODE=${SLURM_MEM_PER_NODE:-unset} SLURM_MEM_PER_CPU=${SLURM_MEM_PER_CPU:-unset}"

python scripts/scan_wse2_ivc_hysteresis_linecut.py \
