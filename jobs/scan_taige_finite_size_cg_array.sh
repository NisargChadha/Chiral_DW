...
echo "Resources: SLURM_CPUS_PER_TASK=${SLURM_CPUS_PER_TASK:-unset} VERTEX_WORKERS=${VERTEX_WORKERS} EXCHANGE_WORKERS=${EXCHANGE_WORKERS} DENSITY_VERTEX_RETENTION=${DENSITY_VERTEX_RETENTION} DENSITY_VERTEX_LAYOUT=${DENSITY_VERTEX_LAYOUT} SLURM_MEM_PER_NODE=${SLURM_MEM_PER_NODE:-unset} SLURM_MEM_PER_CPU=${SLURM_MEM_PER_CPU:-unset}"
...
echo "sbatch --export=ALL,OUTPUT_ROOT=${OUTPUT_ROOT} jobs/merge_taige_finite_size_cg.sh"
