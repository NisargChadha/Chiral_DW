...
for raw_nk in "${NKS[@]}"; do
  ...
  export_arg="ALL,OUTPUT_ROOT=${OUTPUT_ROOT},N_K_LIST=${nk},U_D_MIN=${U_D_MIN},U_D_MAX=${U_D_MAX},N_U_D=${N_U_D},THETA_MIN_DEG=${THETA_MIN_DEG},THETA_MAX_DEG=${THETA_MAX_DEG},N_TWIST=${N_TWIST},VERTEX_WORKERS=${VERTEX_WORKERS},EXCHANGE_WORKERS=${EXCHANGE_WORKERS}"
  cmd=(
    sbatch
    "--array=${ARRAY_SPEC}"
    "--mem=${mem_gb}G"
