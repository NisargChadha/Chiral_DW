#!/usr/bin/env python3
"""Sweep finite-LL AC projected HF response over second harmonics b2 and u2."""
...
class ACSweepPoint:
    ...
    @property
    def label(self) -> str:
        return f"b2_{self.b_index:03d}_u2_{self.u_index:03d}"

    def as_row(self) -> dict[str, Any]:
        return {
            "b_index": int(self.b_index),
            "u_index": int(self.u_index),
            "b2_index": int(self.b_index),
            "u2_index": int(self.u_index),
            "b1": float(self.b1),
            "u1": float(self.u1),
            "b2": float(self.b2),
            "u2": float(self.u2),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-root", default="results/ac_projected_hf_b2_u2_sweep")

    parser.add_argument("--b1", "--b1-fixed", dest="b1", type=float, default=0.0, help="Fixed first magnetic harmonic.")
    parser.add_argument("--u1", "--u1-fixed", dest="u1", type=float, default=0.0, help="Fixed first scalar harmonic.")
    parser.add_argument("--b2", type=float, default=None, help="Run one explicit b2 second-harmonic value.")
    parser.add_argument("--u2", type=float, default=None, help="Run one explicit u2 second-harmonic value.")
    parser.add_argument("--b2-min", type=float, default=-0.3)
    parser.add_argument("--b2-max", type=float, default=0.3)
    parser.add_argument("--n-b2", type=int, default=11)
    parser.add_argument("--u2-min", type=float, default=-0.3)
    parser.add_argument("--u2-max", type=float, default=0.3)
    parser.add_argument("--n-u2", type=int, default=11)
...
def _linspace_points(args: argparse.Namespace) -> list[ACSweepPoint]:
    ...
    if args.b2 is not None and args.u2 is not None:
        return [
            ACSweepPoint(
                b_index=0,
                u_index=0,
...
def _write_sweep_arrays(output_root: Path, rows: list[dict[str, Any]]) -> None:
    ...
    arrays: dict[str, np.ndarray] = {
        ...
        "b2": np.full(shape, np.nan, dtype=float),
...
def _write_plan(output_root: Path, points: list[ACSweepPoint], args: argparse.Namespace) -> None:
    ...
    _write_json(
        output_root / "sweep_plan.json",
        {
            ...
            "sweep_convention": "b_index/u_index enumerate b2/u2; b1/u1 are fixed first harmonics",
            "args": {
                key: value
...
def run_point(args: argparse.Namespace, output_root: Path, point: ACSweepPoint) -> dict[str, Any]:
    ...
    print(
        ...
        f"b2={point.b2:.8g} u2={point.u2:.8g} "
