#!/usr/bin/env python3
"""Run the local conjugate-AC projected-HF b2-u2 cG phase diagram."""
...
SCAN_SCRIPT = ROOT / "scripts" / "scan_ac_projected_hf_b2_u2.py"
PLOT_SCRIPT = ROOT / "Plots" / "plot_ac_b2_u2_cg_phase_diagram.py"


class LocalACB2U2SweepParams(BaseModel):
    """Frozen controls for the laptop-scale second-harmonic AC sweep."""
    ...
    b2_min: float = -0.1
    b2_max: float = 0.1
    n_b2: int = Field(default=11, ge=2)
    u2_min: float = -0.1
    u2_max: float = 0.1
    n_u2: int = Field(default=11, ge=2)
    ...
    def _ranges_and_active_band_are_valid(self) -> "LocalACB2U2SweepParams":
        if self.b2_max <= self.b2_min:
            raise ValueError("b2_max must exceed b2_min")
        if self.u2_max <= self.u2_min:
            raise ValueError("u2_max must exceed u2_min")
...
def _build_parser() -> argparse.ArgumentParser:
    defaults = LocalACB2U2SweepParams()
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output-root", type=Path, default=defaults.output_root)
    parser.add_argument("--plot-output", type=Path, default=defaults.plot_output)
    parser.add_argument("--n-b2", type=int, default=defaults.n_b2)
    parser.add_argument("--n-u2", type=int, default=defaults.n_u2)
...
def _write_run_config(params: LocalACB2U2SweepParams, *, dry_run: bool) -> Path:
        ...
        json.dumps(
            {
                ...
                "physics": {
                    ...
                    "fixed_first_harmonics": {"b1": params.b1, "u1": params.u1},
...
def _run_parallel_sweep(params: LocalACB2U2SweepParams) -> None:
    ...
    n_tasks = params.n_b2 * params.n_u2

    def run_task(task_id: int) -> tuple[int, Path]:
        log_path = log_dir / f"task_{task_id:03d}.log"
        command = params.scan_command(dry_run=False, task_id=task_id, no_write_plan=True)
