...
class SweepPoint:
    """One displacement/twist point in the cluster sweep."""
...
def _linspace_points(args: argparse.Namespace) -> list[SweepPoint]:
    ...
    return [
        SweepPoint(u_index=iu, theta_index=it, u_D=float(u), theta_deg=float(theta))
        for iu, u in enumerate(u_values)
        for it, theta in enumerate(theta_values)
    ]


def _selected_points(args: argparse.Namespace) -> list[SweepPoint]:
...
def _params_for_point(args: argparse.Namespace, point: SweepPoint, point_dir: Path) -> ContinuumWorkflowParams:
...
def _point_dir(output_root: Path, point: SweepPoint) -> Path:
    return output_root / "points" / point.label


def _row_from_point_result(point: SweepPoint, point_dir: Path, result: Any, seconds: float) -> dict[str, Any]:
    refs = result.reference_summary
    return {
        "u_index": point.u_index,
        "theta_index": point.theta_index,
        "u_D_meV": point.u_D,
        "theta_deg": point.theta_deg,
        "cG": float(result.summary.cG),
        "K_min": float(result.summary.kappa_min),
        "K_max": float(result.summary.kappa_max),
        "gap_min": float(result.summary.gap_min),
        "valid_local_gap": bool(result.summary.valid_local_gap),
...
def merge_point_summaries(output_root: Path) -> list[dict[str, Any]]:
    ...
    _write_json(output_root / "sweep.json", {"rows": rows, "n_points": len(rows)})
    return rows


def _write_plan(output_root: Path, points: list[SweepPoint], args: argparse.Namespace) -> None:
    rows = [
        asdict(point) | {"label": point.label, "point_dir": str(_point_dir(output_root, point))}
        for point in points
    ]
...
def run_point(args: argparse.Namespace, output_root: Path, point: SweepPoint) -> dict[str, Any]:
    ...
    row = _row_from_point_result(point, point_dir, result, seconds)
    _write_json(
        point_summary,
        {
            "point": asdict(point),
